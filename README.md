...
└── src/
    ...
    └── test/java/               テストコード（JUnit 5）※ main と同じパッケージ構成
```
...
`mvn test` では、入力チェックや金額計算などのロジック、DAO、
「登録したサンプルの JSP とソースが実在するか」「ID が重複していないか」といった
カタログの整合性までを検査します。Tomcat は起動しないので数十秒で終わります。
