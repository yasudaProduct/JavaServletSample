...
サンプルにしにくい話（1 台の localhost では再現できないこと、コードではなく判断の話）は
**座学メモ**という読み物として `/topics` にまとめています。
...
そのまま開きます（設定済みのプロジェクトファイルを同梱しています）。
このリポジトリは **2 プロジェクト構成**なので、**両方チェックしてインポート**してください。

| プロジェクト | 種類 |
| --- | --- |
| `JavaServletSample` | 動的 Web プロジェクト |
| `servlet-sample-shared`（`shared/`） | 共通ライブラリ（Java プロジェクト） |

Maven を使う場合は、共通ライブラリを先に入れる 2 段階になります
（`make build` なら自動で順番に実行します）。
...
├── shared/                      共通ライブラリ（別プロジェクト。WAR の WEB-INF/lib に同梱される）
├── Makefile                     よく使うコマンド
...
└── src/
    ...
    │   │   ├── catalog/         サンプル一覧・座学メモ（目次）の仕組みと定義
    ...
    │   └── webapp/
    │       ├── WEB-INF/
    │       │   ├── views/       画面の JSP（samples/ がサンプル本体、topics/ が座学メモ、error/ がエラーページ）
    │       │   ├── tags/        共通レイアウト（layout / sample / icon …）
    │       │   ├── tlds/        独自タグの定義
    │       │   └── web.xml      アプリ全体の設定
    │       ├── assets/          CSS / JS / Bootstrap（CDN ではなく同梱）
    │       └── META-INF/context.xml
    └── test/java/               テストコード（JUnit 5）※ src/main と同じパッケージ構成
```
...
- 座学メモは `<t:topic>` で囲みます。デモもソースコードも無いので本文だけを書き、
  見出し・関連サンプル・前後リンクは定義（`TopicDefinitions`）から作られます。
...
座学メモ（読み物）を増やす場合も同じ形です。

1. `src/main/webapp/WEB-INF/views/topics/{ID}.jsp` を作る（本文だけ書く）
2. `src/main/java/com/example/servletsample/catalog/TopicDefinitions.java` に 1 件追加する
...
`mvn test` では、入力チェックや金額計算などのロジック、DAO に加えて、
「登録したサンプル・座学メモの JSP とソースが実在するか」「ID が重複していないか」
「座学メモに書いた関連サンプルの ID が実在するか」といったカタログの整合性までを検査します。
Tomcat は起動しないので数十秒で終わります。
追加したら流しておくと、登録漏れにすぐ気付けます。

テストの書き方そのものは、カテゴリ「テスト」のサンプルで解説しています
（計算ロジック / サービス層 / Servlet の 3 段構え）。

```bash
mvn test -Dtest=OrderPricingTest    # 1 クラスだけ流す
```
...
| アクセシビリティ | スマホでの入力、キーボード操作、読み上げへの配慮 |
...
| 複数サーバー・共通化 | サーバーを分けたときの共通処理の切り出し方とビルド |
| 応用・その他 | フィルタ、エラー処理、国際化など |
| テスト | JUnit 5 で単体テストを書く（計算・サービス・Servlet） |

現在は以下の 55 件が入っています。サンプルはこれから追加していきます。
...
| 基本 | **Servlet から直接出力する** | `getWriter` と `getOutputStream` の使い分け。バッファに収まるかどうかで、書いたあとにヘッダを足せるか・書き直せるかが変わることを実際に確かめる |
...
| アクセシビリティ | **スマホで開くキーボードを切り替える** | `type` / `inputmode` / `enterkeyhint` で数字キーボードを出す。`type="number"` を業務で使うと値がどう壊れるかを並べて確認 |
| アクセシビリティ | **自動入力（autocomplete）** | 住所・氏名・パスワードを補完させるトークン。`one-time-code`、iOS の自動大文字化・自動修正の止め方 |
| アクセシビリティ | **ラベルの付け方と入力欄のグループ化** | `label for` / `fieldset` / `legend` / `aria-describedby`。`placeholder` をラベル代わりにしない理由、タップできる大きさ |
| アクセシビリティ | **エラーの伝え方** | 画面の先頭にエラー一覧を出し、フォーカスを移して各項目へ飛ばす。`aria-invalid`、色だけに頼らないメッセージ |
| アクセシビリティ | **マウスを使わずに操作する** | Tab の順番、`tabindex` の使い分け、`:focus-visible`、「本文へスキップ」、モーダルのフォーカストラップ |
| アクセシビリティ | **画面の変化を読み上げで知らせる** | `role="status"` と `role="alert"` の使い分け、進捗バーの `aria-valuenow`、`aria-busy` |
| アクセシビリティ | **表を読み上げと相性よく作る** | `caption` / `th scope` / `aria-sort`。横に長い表をキーボードでもスクロールできるようにする |
| アクセシビリティ | **画像とアイコンの代替テキスト** | `alt` の書き分け、アイコンだけのボタンの `aria-label`、`.sr-only` の使いどころ |
| アクセシビリティ | **色・コントラスト・拡大・動き** | コントラスト比の基準と Bootstrap 4 既定色の実測、色だけに頼らない伝え方、`prefers-reduced-motion` |
| アクセシビリティ | **自分の画面をチェックする手順** | キーボードで一周・200% 拡大・色を抜く・Lighthouse / axe・読み上げ。そのまま使えるチェックリスト付き |
...
| 複数サーバー・共通化 | **共通処理を JAR に切り出す** | 別プロジェクトにして JAR にし、それぞれの WAR に同梱する。クラスごとの実際の読み込み元を一覧にするので「共有ではなく同梱」が目で見える。Eclipse / Ant / Maven のビルド設定を並べて比較 |
| 複数サーバー・共通化 | **インタフェースで切り離す** | JAR だけでは「共通側がアプリの事情を知ってしまう」問題は解けない。共通側はインタフェースだけを持ち、実装は ServiceLoader でアプリ側から差し替える |
| 複数サーバー・共通化 | **共通化できないもの（状態）** | 共通 JAR を別のクラスローダで読み直して「もう 1 台」を作り、`static` の採番が重複する様子と、DB に寄せると重複しない様子を並べる |
...
| テスト | **単体テストの基本（JUnit 5・境界値）** | 注文金額の計算を題材に、準備・実行・検証の 3 段組み、境界値の選び方、`@ParameterizedTest`、`assertThrows`。画面で境界値を試しながらテストコードと見比べられる |
| テスト | **テストダブルでデータベースから切り離す** | リポジトリをインターフェースにして偽物に差し替え、`Clock` を止めて受注番号を固定する。スタブ・フェイク・スパイ・モックの違い、「保存していないこと」の確かめ方 |
| テスト | **Servlet を単体テストする** | Tomcat を起動せずに `doGet` / `doPost` を直接呼ぶ。偽の `HttpServletRequest` / `HttpServletResponse` を作り、forward 先・リダイレクト先・画面に渡した値を検証する |
...
「一覧・検索」「ファイル」「インクリメンタルサーチ」「トランザクション」「テスト」のサンプルは
**H2 Database** を使っています。
...
サンプルは「画面を触って、ソースを読む」形式ですが、Web アプリの話には
**1 台の localhost では再現できないこと**と、**コードではなく判断の話**がいくつもあります。

- サーバを 2 台に増やしたら、ときどきログアウトするようになった
- 同時に 200 人が使い始めたら、画面が返ってこなくなった
- 本番だけ Cookie が消える。本番だけ時刻が 9 時間ずれる
- Servlet に全部書くか、クラスを分けるか

こうした話はデモを作っても「動いているところ」しか見えないため、
読むだけのメモとして別に置いています。
各メモの最後には関連するサンプルへのリンクがあり、「読む → 触る」でたどれます。

現在は以下の 10 件です。

| グループ | メモ | 内容 |
| --- | --- | --- |
| 動く仕組み | **リクエストが届いて返るまで** | DNS・TCP・リバースプロキシ・コネクタ・スレッド・フィルタ・バッファ。症状からどこを見るかを決めるための地図 |
| 動く仕組み | **Servlet コンテナと WAR** | main を書いていないのに動く理由。WAR の中身、`WEB-INF` が公開されない理由、クラスローダ、JSP が Java に変換される仕組み、配備の形 |
| 動く仕組み | **スレッドとプール** | リクエスト 1 本 = スレッド 1 本。遅い画面 1 つが全体を止める理屈、DB コネクションプールとの関係、スレッドダンプの読み方 |
| 動く仕組み | **セッションはどこにあるか** | Cookie で往復しているのは ID だけ。サーバが 2 台になると消える理由と 4 つの対処、入れてよい値・よくない値 |
| 設計と作法 | **どこに何を書くか** | Servlet / Service / DAO の分け方。分ける基準は「何が変わったら直すか」。トランザクション境界と、分けすぎの見分け方 |
| 設計と作法 | **例外とログ** | 入力の誤り・業務上の都合・システムの異常の切り分け。握りつぶさない、原因を捨てない、リクエスト ID で 1 件ぶんをつなげる |
| 設計と作法 | **業務 Web のセキュリティ全体像** | 「入口で信じない・出口で加工する」。出す先ごとの対策一覧、認可漏れ、パスワード、アップロード、依存ライブラリ |
| 設計と作法 | **「遅い」と言われたときに見るところ** | 測ってから直す手順。N+1・全件取得・索引なしの三点セット、往復回数、キャッシュを入れる順番 |
| 本番とこの先 | **localhost と本番の違い** | HTTPS 終端で崩れる値、`X-Forwarded-For`、Cookie 属性、設定の外出し、消えるファイル、タイムゾーン、無停止デプロイ |
| 本番とこの先 | **javax から jakarta へ、そしてフレームワークへ** | 名前空間が変わった理由と移行手順、Spring MVC が Servlet 1 個である話、次に学ぶことの順番 |

追加の手順は **[docs/ADD_SAMPLE.md](docs/ADD_SAMPLE.md#座学メモ読み物を追加する)** にあります。
